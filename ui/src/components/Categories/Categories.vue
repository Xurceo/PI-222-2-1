...
import { fetchCategories } from "../../api/category_api.ts";
...
setup() {
  ...
  onMounted(async () => {
    ...
    categories.value = flattenCategories(categories.value);
...
<template>
  ...
  <div class="m-10 justify-center text-black" v-if="categories">
    ...
    <ul class="flex flex-col items-start">
      <li class="relative before:content-['▸']" v-for="cat in categories" :key="cat.id">
